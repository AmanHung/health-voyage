...
import {MedicationManager,MedicationHistory} from './medication';
...
export function AdminDirectory({auth,today,patients,onPatientChanged,onReload,onPhoto}:{auth:Auth;today:string;patients:AdminPatient[];onPatientChanged:(p:AdminPatient)=>void;onReload:()=>Promise<void>;onPhoto:(r:RecordItem)=>void}) {
  const [trash,setTrash]=useState(false),[recordTrash,setRecordTrash]=useState(false),[selectedId,setSelectedId]=useState(''),[goalId,setGoalId]=useState(''),[medId,setMedId]=useState('');
  ...
  const epoch=useRef(0),panel=useRef<HTMLElement>(null),goalPanel=useRef<HTMLDivElement>(null),medPanel=useRef<HTMLDivElement>(null),submitting=useRef(false);
  ...
  useEffect(()=>{if(medId)medPanel.current?.scrollIntoView({block:'start'});},[medId]);
  useEffect(()=>{if(goalId)goalPanel.current?.scrollIntoView({block:'start'});},[goalId]);
  async function loadRecords(p:AdminPatient){const current=++epoch.current;setSelectedId(p.id);setGoalId('');setMedId('');setRecords([]);setRecordTrash(false);setLoading(true);setError('');try{const r=await api<{records:RecordItem[]}>(auth,'admin.records',{patientId:p.id});if(current===epoch.current)setRecords(r.records);}catch(e){if(current===epoch.current)setError(e instanceof Error?e.message:'紀錄載入失敗。');}finally{if(current===epoch.current)setLoading(false);}}
  async function reload(){epoch.current++;setSelectedId('');setGoalId('');setMedId('');setRecords([]);setLoading(true);setError('');try{await onReload();}catch(e){setError(e instanceof Error?e.message:'名冊載入失敗。');}finally{setLoading(false);}}
  function ask(value:Omit<Confirmation,'requestId'>){setConfirmError('');setConfirmation({...value,requestId:crypto.randomUUID()});}
  async function changeLeaderboard(p:AdminPatient,enabled:boolean){
    if(submitting.current)return;submitting.current=true;setBusy(true);setError('');setNotice('');
    try{const result=await api<{patient:AdminPatient}>(auth,'admin.leaderboard',{patientId:p.id,enabled,previousVersion:p.leaderboardVersion||null});onPatientChanged(result.patient);setNotice(enabled?'已加入步數排行榜，有步數紀錄後即可排名。':'已取消加入步數排行榜，原有紀錄保留。');}
    catch(e){setError(e instanceof Error?e.message:'排行榜設定未完成，請重新整理名冊確認後重試。');}
  ...
  async function apply(){
    ...
    try{
      if(target.type==='patient'){
        ...
        epoch.current++;setLoading(false);onPatientChanged(result.patient);setSelectedId('');setGoalId('');setMedId('');setRecords([]);
      }else{
  ...
  return <><section className="surface admin-directory"><div className="admin-heading"><h2>個案名冊</h2><Button disabled={busy||loading} variant="ghost" onClick={()=>void reload()}><RefreshCw/>重新整理名冊</Button></div><p className="admin-meta">勾選後自動儲存；測試成員也可加入。取消只會從排行榜隱藏，紀錄保留。個案重新載入首頁後更新榜單。</p><AdminExport auth={auth} today={today} disabled={busy||loading}/><div className="prod-actions admin-filters"><Button disabled={busy} variant={trash?'outline':'default'} aria-pressed={!trash} onClick={()=>setTrash(false)}>使用中個案（{patients.filter(p=>!p.deletedAt).length}）</Button><Button disabled={busy} variant={trash?'default':'outline'} aria-pressed={trash} onClick={()=>setTrash(true)}><Trash2/>已刪除個案（{patients.filter(p=>p.deletedAt).length}）</Button></div>
    {error&&<p className="prod-error" role="alert">{error}</p>}{notice&&<p className="prod-success" role="status">{notice}</p>}
    <div className="prod-records">{patients.filter(p=>!!p.deletedAt===trash).map(p=><article key={p.id} className={trash?'admin-deleted':''}><h3>{p.name}</h3><p>{p.bound?'已綁定':'尚未綁定'}・{p.nickname}{p.deletedAt?'・已刪除':!p.active?'・已停用':''}</p>{!trash&&<label className="admin-leaderboard-toggle"><input type="checkbox" checked={p.leaderboardEnabled??!p.isTest} disabled={busy||loading||!p.active} onChange={e=>void changeLeaderboard(p,e.target.checked)} aria-label={`${p.name}：加入步數排行榜`}/><span>加入步數排行榜</span></label>}<div className="prod-actions"><Button disabled={busy||loading} variant="outline" onClick={()=>void loadRecords(p)}>查看紀錄</Button>{!trash&&<Button disabled={busy||!p.active} variant="outline" onClick={()=>{setGoalId(p.id);setMedId('');}}>設定活動目標</Button>}{!trash&&<Button disabled={busy||!p.active} variant="outline" onClick={()=>{setMedId(p.id);setGoalId('');}}>管理用藥</Button>}<Button disabled={busy} variant={trash?'outline':'destructive'} onClick={()=>ask({type:'patient',patient:p,deleted:!trash})}>{trash?<RotateCcw/>:<Trash2/>}{trash?'復原個案':'刪除個案'}</Button></div></article>)}</div>
    {!patients.some(p=>!!p.deletedAt===trash)&&<p>{trash?'目前沒有已刪除個案。':'目前沒有個案。'}</p>}<p className="admin-meta">刪除採可復原方式，原始資料與照片仍保留於私人儲存空間。</p>
    </section>
    {medId&&patients.find(p=>p.id===medId&&!p.deletedAt)&&<div ref={medPanel}><MedicationManager key={medId} auth={auth} today={today} patient={patients.find(p=>p.id===medId)!} isAdmin/></div>}

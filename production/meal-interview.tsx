...
import {ArrowLeft,Check,ChevronRight,HelpCircle,Utensils} from 'lucide-react';
...
export function MealInterview({imageUrl,period,onPeriod,value,onChange,onReady}:{imageUrl:string|null;period:string;onPeriod:(v:string)=>void;value:Answers;onChange:(v:Answers)=>void;onReady:(v:boolean)=>void}){
  const [screen,setScreen]=useState(value.mealType?2:0),[suggestion,setSuggestion]=useState<MealSuggestion|null>(null),[analyzing,setAnalyzing]=useState(false),[manual,setManual]=useState(value.source==='patient-entered'),[manualName,setManualName]=useState(value.mealName||'');
  ...
  useEffect(()=>{
    ...
    let active=true;setAnalyzing(true);setSuggestion(null);
    const image=new Image();image.onload=()=>classifyMeal(image).then(result=>{if(active)setSuggestion(result);}).catch(()=>{if(active)setSuggestion(null);}).finally(()=>{if(active)setAnalyzing(false);});image.onerror=()=>{if(active)setAnalyzing(false);};image.src=imageUrl;
    return()=>{active=false;};
  ...
  function identify(type:MealType,name:string=type,source:Answers['source']='patient-selected'){
    update({...emptyMealInterview(),mealType:type,mealName:name.trim().slice(0,40),source,modelSuggestion:suggestion?.name||''});setScreen(2);setManual(false);
  }
  ...
  function resetType(){onChange(emptyMealInterview());setManual(false);setManualName('');setScreen(1);}
  const progressTotal=(keys.length||7)+2,progress=Math.min(screen+1,progressTotal);
  if(screen===0)return <Question title="這是哪一餐？" current={1} total={progressTotal}><OptionGrid options={PERIODS} value={period} onPick={v=>{onPeriod(v);setScreen(1);}}/></Question>;
  if(screen===1)return <Question title={analyzing?'正在看照片…':suggestion?`這看起來像「${suggestion.name}」嗎？`:'這是什麼餐點？'} current={2} total={progressTotal}>
    {analyzing?<div className="meal-analyzing"><Utensils/><p>第一次使用會稍等一下</p></div>:suggestion&&!manual?<div className="meal-answer-row"><Button type="button" onClick={()=>identify(suggestion.mealType,suggestion.name,'model-confirmed')}><Check/>是</Button><Button type="button" variant="outline" onClick={()=>setManual(true)}>不是</Button><Button type="button" variant="ghost" onClick={()=>setManual(true)}><HelpCircle/>不知道</Button></div>:<IdentifyChoices manualName={manualName} onManualName={setManualName} onManual={()=>{const name=manualName.trim();if(name)identify(inferMealType(name),name,'patient-entered');}} onPick={type=>identify(type,type,'patient-selected')}/>} 
  </Question>;
...
function IdentifyChoices({manualName,onManualName,onManual,onPick}:{manualName:string;onManualName:(v:string)=>void;onManual:()=>void;onPick:(v:MealType)=>void}){
  return <div className="meal-identify"><label htmlFor="meal-name">直接輸入餐點名稱</label><Input id="meal-name" value={manualName} maxLength={40} placeholder="例如：雞腿便當" onChange={e=>onManualName(e.target.value)} onKeyDown={e=>{if(e.key==='Enter'){e.preventDefault();onManual();}}}/><Button type="button" disabled={!manualName.trim()} onClick={onManual}>使用這個名稱<ChevronRight/></Button><p>或選最接近的類型</p><div className="meal-type-grid">{MEAL_TYPES.map(type=><Button type="button" variant="outline" key={type} onClick={()=>onPick(type)}><span aria-hidden>{icons[type]}</span>{type}</Button>)}</div></div>;
}

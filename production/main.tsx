...
function RecordForm({auth,kind,today,initialDate,record,onSaved}:{auth:Auth;kind:RecordItem['kind'];today:string;initialDate?:string;record?:RecordItem;onSaved:(r:RecordItem)=>void}){
  const [date,setDate]=useState(record?.date||initialDate||today),[value,setValue]=useState(record?.mode==='steps'?record.value?.toString()||'':'');
  ...
  const photoReady=kind==='medicine'||Boolean(prepared?.dataUrl||record?.hasImage);
  ...
  async function submit(e:FormEvent){e.preventDefault();if(!photoReady){setError('請先上傳照片；辨識失敗仍可手動填寫並儲存。');return;}if(busy||processing||(kind==='meal'&&(!mealReady||!mealInterviewComplete(meal))))return;reader.current?.cancel();reader.current=null;setReading(false);setBusy(true);setError('');try{
  ...
  return <form className="prod-form" onSubmit={submit}><fieldset disabled={busy||processing} className="prod-form">
    ...
    {kind!=='medicine'&&<><input ref={fileInput} className="sr-only" type="file" accept="image/jpeg,image/png" onClick={e=>{e.currentTarget.value='';}} onChange={e=>void select(e.target.files?.[0])}/><Button type="button" variant="outline" onClick={()=>fileInput.current?.click()}><Camera/>{kind==='meal'&&(record?.hasImage||prepared)?'更換餐點照片':kind==='meal'?'選擇餐點照片':'選擇運動截圖'}</Button>{prepared?kind==='meal'?<details className="meal-photo-strip"><summary><img src={prepared.preview} alt=""/><span>已選擇餐點照片<small>點開查看大圖</small></span></summary><img className="prod-photo" src={prepared.preview} alt="本餐照片預覽"/></details>:<details className="meal-photo-strip"><summary><img src={prepared.preview} alt=""/><span>已選擇運動截圖<small>已縮小為 {Math.round(prepared.bytes/1024)} KB，點開查看</small></span></summary><img className="prod-photo" src={prepared.preview} alt="運動截圖預覽"/></details>:record?.hasImage?<p>目前保留原照片，可按上方按鈕更換。</p>:<p>必須上傳照片才能完成紀錄。辨識失敗時，可手動填寫，照片會一併保存。</p>}</>}
    ...
    </fieldset>{processing&&<p role="status">正在處理圖片…</p>}{ocrText&&<p role="status">{ocrText}</p>}{reading&&<Button type="button" variant="outline" onClick={manualSteps}>不用等待，手動填寫步數</Button>}{error&&<p className="prod-error" role="alert">{error}</p>}<Button type="submit" disabled={!photoReady||busy||processing||(kind==='meal'&&!mealReady)||!mealInterviewComplete(meal)&&kind==='meal'}>{busy?'儲存中…':'儲存紀錄'}<Check/></Button></form>;
}
